...
export PROGRAM_NAME="CopyingRunner"
...
export TMP_DIR_FOR_COPYING="/tmp"
export  TMP_SOURCE_FILE="${TMP_DIR_FOR_COPYING}/"$(basename "$SOURCE_FILE")
...
__copy_runner_file ()
{
    ACTION="Copy Runner File"
    __show_running
    echo "  -------- Copying ${SOURCE_FILE} to ${TMP_DIR_FOR_COPYING}. --------" >> "${LOGFILE}"
    if /usr/local/share/apps/copyfile.py "${SOURCE_FILE}" "${TMP_DIR_FOR_COPYING}"; then
        echo "  -------- Copied ${SOURCE_FILE} to ${TMP_DIR_FOR_COPYING}. --------" >> "${LOGFILE}"
        __close_progressbar
    else
        __cleanup
        __quit_on_error "Could not copy runner file into /tmp to perform update. Maybe USB disk problem?"
    fi
}
...
if [ -f "${SOURCE_FILE}" ]; then
    ...
    if [ -r "${SOURCE_FILE}" ]; then
        __copy_runner_file
        __unpack_it "${TMP_SOURCE_FILE}" "${CACHE_DIR}"
