...
EmergencyGrubFile = 'grub.cfg.OSprimarySingleDisk'
MirrorFilename = '/usr/local/etc/mirror/mirror.cfg'
SingleDiskMirrorFile = 'mirror.cfg.OSprimarySingleDisk'
        ...
        if "config_filename" in self.params_dict:
            try:
                ...
                try:
                    ...
                    pass
                try:
                    self.config_file_params_dict["single_disk_system"] = \
                        config.get("System", "single_disk_system")
                except configparser.NoOptionError:
                    pass
                val = None
                try:
        ...
        try:
            ...
            try:
                ...
                if len(re.findall(r'result:\s+(fail)', check_result,
                    ...
                    command = '/usr/local/sbin/smartctl -a ' + self.device
...
class SystemChecker:
    ...
    def __init__(self, reporter, config):
        ...
        self.cleanup_info_logger = None
        self.delete_files_info_logger = None
    ...
    def discover_disks(self):
        ...
        try:
            if not self.config.get_value("single_disk_system") and len(system_disks) > 1:
                self.disks[system_disks[1]] = DiskInfo("Backup Disk", system_disks[1], False, self)
            else:
                self.handle_missing_disk()
        except OSError:
            pass
            ...
            if data_values[2] in required_filesystems:
                ...
                self.partition_free_space[data_values[2]] = \
                    {"percent": percent, "amount": round(avail / 1.0e6, 1),
                     ...
                     "display used": localFunctions.convert_to_readable(used, True),
                     "display avail": localFunctions.convert_to_readable(avail, True)}
                if percent < 10.0:
    ...
    def check_openvpn(self):
        """
        This is called only when the internet is alive so that we can be guaranteed
        that openvpn should be active and provideing a tun0 interface. If not, then
        it may have crashed, so attempt a single restart.
        :return:
        """
        if "tun0" in self.network_interfaces and \
            not self.network_interfaces["tun0"].up:
            self.reporter.report_fixable_problem("openvpn test failed")
            self.restart_failed_process("openvpn")
            time.sleep(4)
            self.network_interfaces["tun0"].get_status()
            if self.network_interfaces["tun0"].up:
                self.reporter.report_fix_result("openvpn restart successful")
            else:
                self.reporter.report_requires_user_action_problem(
                    error_message_name="openvpn restart failed",
                    action_message_name="",
                    increment_problem_count=False)
        return ("tun0" in self.network_interfaces and
    ...
    def handle_missing_disk(self):
        if not self.config.get_value("single_disk_system") and len(self.disks) < 2:
            disk_name = "primary" if self.get_using_backup_disk() else "backup"
    ...
    def root_partition_too_full(self, free_space_required):
        """
        A simple function to check size and return True if the root partition
    ...
    def handle_partitions_full(self):
        ...
        try:
            if self.partition_full:
                self.cleanup_info_logger = localFunctions.create_timestamped_logger(
                    "InfoLogger", "/var/log/cleanup/info.log")
                self.delete_files_info_logger = localFunctions.create_timestamped_logger(
                    "DeleteLogger", "/var/log/cleanup/delete_info.log")
                if self.root_partition_too_full(10.0):
                    ...
                    if self.root_partition_too_full(10.0):
                        self.reporter.report_problem("os partition full", values= [
                                                     self.partition_free_space['/']['display avail'],
                                                     self.partition_free_space['/']["percent"]])
                if ('/client_home' in self.partition_free_space and
                    ...
                    if self.partition_free_space['/client_home']["amount"] < 10.0:
                        initial_partition_free_space = self.partition_free_space['/client_home']["avail"]
                        max_file_size_str = "500MB"
                        max_file_size =500e6
                        self.reporter.report_fixable_problem("emergency clean client_home",
                             values= [self.partition_free_space["/client_home"]["display avail"],
                                     self.partition_free_space['/client_home']["percent"]])
                        self.reporter.report_starting_fix("emergency cleaning teachers",
                              values=[max_file_size_str])
                        systemCleanup.clean_client_home_teachers_files(max_file_size,
                                                               logger=self.delete_files_info_logger)
                        self.check_partitions_free_space()
                        final_partition_free_space = self.partition_free_space['/client_home']["avail"]
                        delta = final_partition_free_space - initial_partition_free_space
                        self.reporter.report_fix_result("cleaning result", values=[
                            localFunctions.convert_to_readable(delta, True), "/client_home",
                            self.partition_free_space["/client_home"]["display avail"],
                            self.partition_free_space["/client_home"]["percent"]])
                ...
                if ('/client_home_students' in self.partition_free_space and
                    self.partition_free_space['/client_home_students']["amount"] < 10.0):
                    initial_size = localFunctions.get_filesystem_space_used("/client_home")
                    initial_partition_free_space = self.partition_free_space['/client_home_students'][
                        "avail"]
                    self.reporter.report_fixable_problem("emergency clean client_home_students",
                         values=[self.partition_free_space['/client_home_students']["display avail"],
                                 self.partition_free_space['/client_home_students']["percent"]])
                    self.reporter.report_starting_fix("emergency cleaning client_home_students",
                                                      values=[])
                    max_file_size = {"video": 5e6, "audio": 1e6, "photo": 8e5, "other": 1}
                    dirs_removed, files_removed, invalid_files = \
                        serveStudentUseWeb.noninteractive_clean_students_directories(max_file_size,
                                                             logger=self.delete_files_info_logger)
                    self.check_partitions_free_space()
                    size_change_message, delta = localFunctions.change_in_filesystem_size(
                        "/client_home_students",
                        initial_size)
                    self.reporter.report_fix_result("client_home_students cleaning result",
                                values=[files_removed, size_change_message,
                                self.partition_free_space["/client_home_students"]["display avail"],
                                self.partition_free_space["/client_home_students"]["percent"]])
                    if invalid_files:
                        self.cleanup_info_logger.info(
                            """System Check removed %d files and %d directories in /client_home_students.
                            %s""" %(files_removed, dirs_removed, size_change_message))
                if self.partition_free_space['/client_home_students']["amount"] < 20.0:
                    report_text, html_report_text = get_accounts_usage(count=4,
                    ...
                    self.reporter.report_requires_user_action_problem(
                        ...
                        values=[
                            self.partition_free_space['/client_home_students']["display avail"],
                            self.partition_free_space['/client_home_students']["percent"]],
                        action_message_name="run Cleanup Student Files app",
                        action_values=[report_text])
        except Exception as e:
            self.function_errors["handle_partitions_full"] = str(e)
    # ----------------------------------------------------------------------
    def cleanup_root_partition(self):
        ...
        complete failure.
        This is a lighter weight action that is done to perhaps prevent a warning.
        :return:
        """
        systemCleanup.clean_os_copies("/OS_Copies")
        systemCleanup.clean_opt(remove_alt_image=False)
        ...
        systemCleanup.clean_dir("/var/crash", exclusions=[], prune_active_owner=False,
                                other_protected_users=[])
        systemCleanup.clean_media(exclusions=[], prune_active_owner=True, other_protected_users=[])
    ...
    def emergency_cleanup_root_partition(self):
        ...
        try:
            target_size = 10
            self.reporter.report_fixable_problem("os partition full", values=[
                self.partition_free_space["/"]["display avail"],
                self.partition_free_space["/"]["percent"]])
            self.reporter.report_starting_fix("clean os partition")
            ...
            if self.root_partition_too_full(target_size):
                self.clean_var()
            if self.root_partition_too_full(target_size):
                systemCleanup.clean_media(exclusions=[],
            ...
            self.function_errors["emergency_cleanup_root_partition"] = str(e)
    ...
    def clean_sysadmin_and_master(self):
        ...
        try:
            ...
            media_files_by_class, media_size_by_class, oversize_files, \
                ...
                fileManagementFunctions.get_media_files( "/home/sysadmin/",
                {"video": 1e7, "audio": 3e6, "photo": 1e6, "other": 1})
            serveStudentUseWeb.delete_invalid_files(oversize_files,
                                                    logger=self.delete_files_info_logger)
    ...
    def check_sysadmin_home_size(self):
        ...
        home_size_text = localFunctions.convert_to_readable(home_size, True)
        if home_size > 600:
            self.reporter.report_requires_user_action_problem(
                error_message_name="sysadmin home excessive problem",
                values=[home_size_text],
                action_message_name="sysadmin home excessive action",
                action_values=[home_size_text])
        elif home_size > 330:
            self.reporter.report_requires_user_action_problem(
                error_message_name="", values=[],
    ...
    def analyze_load_minutes(self):
        ...
        if self.load_monitor_minutes:
            ...
            if above70 > 0.1:
                self.reporter.report_requires_user_action_problem("load high",
                                                            values=[70,
...
def report_errors(error_dict):
    ...
    if error_report_string and error_logger:
